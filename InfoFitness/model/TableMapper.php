...
class TableMapper {
    ...
    public function listTables() {
        ...
        foreach ($list_db as $table) {
            array_push($tables, new Table($table["id_tabla"],$table["nombre_tabla"], $table["descripcion"]));
        }
    ...
    public function fechTable($id_tabla) {
      ...
      if($table != null) {
        return new Table($table["id_tabla"],$table["nombre_tabla"],$table["descripcion"]);
      }
    }

    public function fechExercisesTable($id_tabla) {
      $stmt = $this->db->prepare("SELECT ejercicio.nombre, ejercicio.id_ejercicio FROM tabla_ejercicios_detalles inner join ejercicio on
